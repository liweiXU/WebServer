...
#define BUFFER_SIZE 4096 //设置读缓冲区的大小

//主状态机的两种可能状态，分别表示: 当前正在分析请求行，当前正在分析头部字段
enum  CHECK_STATE { CHECK_STATE_REQUESTLINE = 0, CHECK_STATE_HEADER };

//从状态机的三种可能状态，即行的读取状态，分别表示: 读取到一个完整的行，行出错和行数据不完整
...
enum HTTP_CODE { NO_REQUEST, GET_REQUEST, BAD_REQUEST, FORBIDDEN_REQUEST, INTERNAL_ERROR,CLOSE_REQUEST };

// 为了简化问题，我们没有给客户端发送一个完整的HTTP应答报文，而只是根据服务器的处理结果发生成功或失败信息；
...
{
    LINE_STATUE linestatus = LINE_OK; //记录当前行的读取状态
